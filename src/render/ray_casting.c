...
float	horizontal(t_map *map, t_pnt pnt, float alp) {
	t_pnt	A;
	t_pnt	dlta;
	...
	while (A.y >= 0 && A.y < map->h) {
		if (A.x >= 0 && A.x < map->w)
			if (map->box[(int)A.y][(int)A.x] != 0) // if box
				break ;
			else {
				A.y += dlta.y;
				A.x += dlta.x;
...
float	vertical(t_map *map, t_pnt pnt, float alp) {
	t_pnt	A;
	t_pnt	dlta;
...
float	ray_casting(t_map *map, t_pnt pnt, float alp) {
	float hor = horizontal(map, pnt, alp);
	float ver = vertical(map, pnt, alp);
